...
class MognoURI {
  ...
  parseObject(uriObject) {
    ...
    let uri;
    if (uriObject.scheme) {
      if (!['mongodb', 'mongodb+srv'].includes(uriObject.scheme)) {
        throw new Error('schema should be mongodb or mongodb+srv');
      }
      uri = `${uriObject.scheme}://`;
    } else {
      uri = 'mongodb://';
    }
  ...
  parseString(uri) {
    ...
    if (!['mongodb', 'mongodb+srv'].includes(uriObject.scheme)) {
      throw new Error('URI must begin with mongodb:// or mongodb+srv://');
    }
    ...
    if (i >= 0) {
      ...
      options.split('&').forEach(function (o) {
        ...
        const optionsKey = decodeURIComponent(o.substring(0, iEquals));
        const optionsValue = decodeURIComponent(o.substring(iEquals + 1));
        uriObject.options[optionsKey] = optionsValue;
      });
    }
