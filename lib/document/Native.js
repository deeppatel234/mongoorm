...
class NativeMethods {
   ...
   */
  getCollection () {
    if (_.isUndefined(this.documentName)) {
      throw new MongoError.DocumentNotDefine()
    }
    return db.get().collection(this.documentName)
  }
  /**
   ...
   */
  find (domain, options) {
    return this.getCollection().find(domain, options).toArray()
  }
  /**
  ...
  findOne (domain, options) {
    return this.getCollection().findOne(domain, options).toArray()
  }
  /**
