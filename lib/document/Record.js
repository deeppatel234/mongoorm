...
class Record {
  ...
  _saveRecord() {
    ...
    if (validate.isValid) {
      ...
      return new Promise((resolve, reject) => {
        self.document.insertOne(data, (err, result) => {
          if (err) {
            logger.error(err);
            reject(err);
          } else {
    ...
    return Promise.reject(validate.errorMessage);
  }
  /**
  ...
  _update() {
    ...
    if (validate.isValid) {
      ...
      return this.document.updateOne({ _id: this.getId() }, { $set: data }).then((err) => {
        if (!err) {
          self._triggerSaved(data);
        }
      });
   ...
   */
  delete() {
    return this.document.deleteOne({ _id: this.getId() });
  }
  /**
   * return record id
   * @memberof Record
   ...
   */
  validate() {
    if (this.options.validateBefourSave && !this.ele.validate()) {
