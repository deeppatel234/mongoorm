...
class Record {
  ...
  _update() {
    ...
    return self.validate().then(() => {
      let data = self._getFieldsValue('modified', false, false);
      if (_.isEmpty(data)) {
        return Promise.resolve();
      }
   ...
   */
  save() {
    return this.ele.beforeSave().then(() =>
      this.document.wrapHooks('save', this, () => (this.getId() ? this._update() : this._saveRecord())));
  }
  /**
   * validate data
   * @returns boolean
