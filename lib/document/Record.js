...
class Record {
  constructor(document, data = {}, options = {}) {
    ...
    const ClassName = document.fields.fieldClass;
    this.ele = new ClassName(document.fields.props);
   ...
   */
  _getFieldsValue({
    mode = 'all', id = true, timestamp = true,
  } = {}) {
    let data = mode === 'modified' ? this.ele.getModifiedData() : this.ele.get();
    let omitedFields = [];
    if (!id) {
  ...
  /**
   * prepare fields data before save record
   *
   * @returns promises
   * @memberof Record
   */
  ...
  _saveRecord() {
    const self = this;
    return this._prepareSaveRecord().then(function (data) {
  ...
  _update(query) {
    ...
    return self.validate().then(() => {
      ...
      query = query || { _id: self.getId() };
      return self.document.updateOne(query, { $set: data }).then((err) => {
        if (!err) {
          self._triggerSaved(data);
          return data;
        }
   ...
   * @memberof Record
   */
  _updateModified(query) {
    const self = this;
    return self.validateModified().then(() => {
      let data = self._getFieldsValue({ mode: 'modified', id: false, timestamp: false });
      if (_.isEmpty(data)) {
        return Promise.resolve();
      }
      ...
      data = mongoutils.obj2dot(data);
      query = query || { _id: self.getId() };
      return self.document.updateOne(query, { $set: data }).then((res) => {
        self._triggerSaved(data);
        return { res, data };
      });
    });
   ...
   */
  save(query) {
    return this.document.wrapHooks('save', this, () => (this.getId() ? this._update(query) : this._saveRecord()));
  }
   ...
   */
  update(query) {
    return this.document.wrapHooks('save', this, () => this._updateModified(query));
  }
