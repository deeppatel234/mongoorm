...
class ObjectField extends AbstractFields {
  constructor(props) {
    ...
    this.elements = _.cloneDeep(props.ele);
  ...
  set(value) {
    this.recursiveSet(this.elements, value);
    super.set(value);
  }

  get() {
    let value = {};
    this.recursiveGet(this.elements, value);
    return value;
  }

  recursiveGet(ele, value) {
    ...
    for (let i = 0; i < keys.length; i += 1) {
      ...
      if (ele[key].isFieldObject) {
        ...
        this.recursiveGet(ele[key], value[key]);
    ...
    for (let i = 0; i < keys.length; i += 1) {
      ...
      if (!_.isUndefined(value[key])) {
        if (ele[key].isFieldObject) {
          ...
          this.recursiveSet(ele[key], value[key]);
        }
      }
    }
  }

  recursiveValidate(ele) {
    let keys = Object.keys(ele);
    for (let i = 0; i < keys.length; i += 1) {
      let key = keys[i];
      if (ele[key].isFieldObject) {
        ...
        return this.recursiveValidate(ele[key]);
      }
    }
    return true;
  }
  ...
  validate() {
    ...
    if (res) {
      res = this.recursiveValidate(this.elements);
    }
    return res;
  }
...
class ArrayFields extends AbstractFields {
  ...
  _getDefaultValue(propValue) {
    return this.getEleObject(super._getDefaultValue(propValue));
  }
  ...
  validate() {
    ...
    if (res) {
      for (let i = 0; i < this._value.length; i += 1) {
        if (!this._value[i].validate()) {
          this.errorMessages.push(...this._value[i].getErrorMessage(`{KEY} : ${i} index`));
          res = false;
          break;
        }
      }
